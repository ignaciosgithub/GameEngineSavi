...
# Build the simplified emergency renderer
g++ -std=c++11 -o bin/linux/Editor_emergency_simple \
    Editor/Editor_emergency_simple.cpp \
    Editor/Editor.cpp \
...
if [ $? -eq 0 ]; then
    echo "Simplified emergency renderer build successful."
    chmod +x bin/linux/Editor_emergency_simple
else
    echo "Simplified emergency renderer build failed."
fi
