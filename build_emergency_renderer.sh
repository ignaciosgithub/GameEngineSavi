...
# Build the emergency renderer
g++ -std=c++11 -o bin/linux/Editor_emergency \
    Editor/Editor_emergency.cpp \
    Editor/Editor.cpp \
...
if [ $? -eq 0 ]; then
    echo "Emergency renderer build successful."
    chmod +x bin/linux/Editor_emergency
else
    echo "Emergency renderer build failed."
fi
